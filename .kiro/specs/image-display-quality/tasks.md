...
- [ ] 2. Configure environment
    ...
    - Fill in `SMUGMUG_QA_USERNAME`, `SMUGMUG_QA_PASSWORD`, `INSIDE_AUTH_USER`, `INSIDE_AUTH_PASS`
    - Create a dedicated test album in SmugMug and add its key as `TEST_ALBUM_KEY`
    - Set `TEST_NICKNAME` to the test account (e.g., `qa-pro`)
    - _Requirements: all_
...
- [ ] 18. Run the full suite against the inside environment and capture baseline results
    - `npm test 2>&1 | tee baseline-results.txt`
