...
async function loadImage(page: Page, url: string): Promise<Locator> {
  await page.goto(url);
...
for (const { name, width, height } of viewports) {
  test(`image renders and is visible at [${name}] ${width}x${height}`, async ({
    browser,
  }) => {
  ...
  test(`candidate matches baseline at [${name}] ${width}x${height}`, async ({
    browser,
  }) => {
